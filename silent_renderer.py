...
class SilentRenderer:
    ...
    def _get_schema(self, db_path: str) -> list:
        ...
        try:
            ...
            print(f"[SilentRenderer] Could not read schema: {e}")
            return []
    ...
    async def handle_open_database(self, params: dict):
        ...
        # Try to find and render schema from actual DB file
        db_path = None
        if self._storyboard_dir:
            candidate = self._storyboard_dir / "assets" / db_name
            if candidate.exists():
    ...
    async def render_async(self, storyboard_path: Path, output_mp4: Path):
        ...
        # Set storyboard dir for DB schema lookup
        self._storyboard_dir = storyboard_path.parent
